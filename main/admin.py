...
class UserAdmin(DjangoUserAdmin):
    ...
    add_fieldsets =(
        (None,{'classes':("wide",),'fields':("emails","password1","password2")},),
    )
...
class OrderAdmin(admin.ModelAdmin):
    ...
    fieldsets =((None, {"fields": ("user", "status")}),("Billing info",{"fields": ("billing_name","billing_address1",
    "billing_address2","billing_zip_code","billing_city","billing_country",)},),("Shipping info",{ "fields": ("shipping_name",
    "shipping_address1","shipping_address2","shipping_zip_code","shipping_city","shipping_country",)},),)
...
class CentralOfficeOrderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Billing info", {"fields":("billing_name","billing_address1",
    "billing_address2","billing_zip_code","billing_city","billing_country",)},),
...
class DispatchersOrderAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status","shipping_country","date_added")
    inlines = (CentralOfficeOrderLineInline,)
    fieldsets = (("Shipping info", {"fields":("shipping_name","shipping_address1","shipping_address2","shipping_zip_code",
...
class ColoreAdminSite(admin.sites.AdminSite):
    def each_context(self,request):
        context = super().each_context(request)
        context["site_header_color"] = getattr(self,"module_caption_color",None)
        return context
...
class ReportingColoreAdminSite(ColoreAdminSite):
    def get_urls(self):
        urls = super().get_urls()
        my_urls = [
            path(
    ...
    def orders_per_day(self, request):
        ...
        return TemplateResponse(
            request, "main/orders_per_day.html", context
        )
    ...
    def most_bought_products(self, request):
        ...
        return TemplateResponse(
            request, "main/most_bought_products.html", context
        )
...
class DispatchersAdminSite(ReportingColoreAdminSite):
    site_header = "Afia central dispatch administration"

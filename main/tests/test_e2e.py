...
class FrontendTest(StaticLiveServerTestCase):
    ...
    def test_product_page_switches_images_correctly(self):
        product = models.Product.objects.create(name="The cathedral and the bazaar",slug='cathedral-bazaar',price=Decimal('10.00'),)
        for fname in ["cb1.jpg","cb2.jpg","cb3.jpg"]:
            with open("main/fixtures/cb/{}".format(fname), "rb") as f:
                print(f)
                image = models.ProductImage(product=product,image=ImageFile(f,fname),)
                image.save()
